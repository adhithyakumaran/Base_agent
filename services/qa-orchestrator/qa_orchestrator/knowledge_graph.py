...
class FlowKnowledgeGraph:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            from repo_paths import automation_dir as default_automation

            default = default_automation()
        except ImportError:
            default = Path("apps/automation") if Path("apps/automation").exists() else Path("automation")
        self.automation_dir = Path(automation_dir or default)
    ...
    def _load_synonyms(self) -> dict[str, list[str]]:
        path = self.discovery_root / "capability_synonyms.yaml"
        if not path.exists():
            return {}
        data = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        return {str(k): [str(x) for x in v] for k, v in (data.get("synonyms") or {}).items()}

    def _expand_query_tokens(self, query: str) -> set[str]:
        q = query.lower()
        tokens = _tokenize(query)
        for cap, syns in self._synonyms.items():
            cap_l = cap.lower()
            if cap_l in q or tokens & _tokenize(cap):
    ...
    def match_capabilities(self, query: str) -> list[str]:
        q = query.lower()
        q_tokens = self._expand_query_tokens(query)
        matched: list[str] = []
        for cap in self._cap_to_flows:
            cap_tokens = _tokenize(cap)
            syns = self._synonyms.get(cap, [])
            syn_tokens: set[str] = set()
            for s in syns:
                syn_tokens |= _tokenize(s)
            if cap.lower() in q or q_tokens & cap_tokens or q_tokens & syn_tokens:
                matched.append(cap)
        return matched

    def flows_for_query_semantic(self, query: str, *, limit: int = 8) -> list[str]:
        """Resolve flows via capability synonyms + graph traversal."""
        caps = self.match_capabilities(query)
        flow_ids: list[str] = []
        for cap in caps:
            flow_ids.extend(self.flows_for_capability(cap))
        flow_ids.extend(self.search_flows(query, limit=limit))
        seen: set[str] = set()
        out: list[str] = []
        for fid in flow_ids:
            if fid in seen:
